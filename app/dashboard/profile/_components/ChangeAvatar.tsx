...
export default function ChangeAvatar({ image }: { image: string | null }) {
  ...
  const [finalImageUrl, setFinalImageUrl] = useState<string | null>(image);
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      // 3. Update user with base64 image data directly
      await authClient.updateUser({
        image: base64Image, // e.g., "data:image/jpeg;base64,/9j/4AAQ..."
      });

      // 4. Revoke old blob URLs
      if (finalImageUrl && finalImageUrl.startsWith("blob:")) {
        URL.revokeObjectURL(finalImageUrl);
      }

      // 5. Set the final avatar to base64 string
      setFinalImageUrl(base64Image);
